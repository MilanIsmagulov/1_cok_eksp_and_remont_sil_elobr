const anwserArr = [['оформление перерывов в работе, перемещения на другое рабочее место и окончание работы'], ['вывешивание запрещающих плакатов на ручных приводах и ключах дистанционного управления'], ['ограничивается внешним осмотром, чисткой, смазкой трущихся деталей и измерением сопротивления контактов постоянному току'], ]; //Ответы в правильном порядке
const text = ["Организационные мероприятия", "Технические <br> мероприятия",  "Текущий ремонт оборудования", ]
...
textOfQuestionPlace.innerHTML = '<p>' + '<span>' + numberOfQuestion +  '. ' + '</span>'  + 'Укажите, какие виды работ относятся к указанным мероприятиям.' + '</p>'  + '<span style="margin-left: 25px; font-size: 25px; font-weight: 700;">  <br></span>'
